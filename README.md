...
ライブ情報に特化した SNS の API です。

## API Document

API ドキュメントは[こちら](https://liveshare-api-document.netlify.app/)
...
```
$ git clone https://github.com/TetsuoYoshikawa/Rese-api.git
```

Rese-api ディレクトリに移動し、vendor ディレクトリがないので入れます。
...
Rese-api ディレクトリ直下にある.env.example の名前を.env に変更します。
.env 内を下記のように編集します。
